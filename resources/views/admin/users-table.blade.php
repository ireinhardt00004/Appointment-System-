...
<section class="bg-white w-full h-full dark:bg-gray-900">
  ...
  <form class="w-full h-full flex" action="{{ route('users-table.search') }}" method="GET">
      @csrf  
          ...
          <thead class="text-xs w-full text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              ...
              <tr>
                  <th colspan="22" class="px-6 py-3">
                      <h2 class="font-bold text-gray">Searched: {{ $query }}</h2>
                  </th>
              </tr>
              @endif --}}
          @if ($hasResults) 
            ...
            <td colspan="9"class="px-6 py-4 font-bold">No users data</td>
             ...
             @else
              ...
              <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                  ...
                  <td class="px-6 py-4">{{ $user->contact_num }}</td>
                  <td class="px-6 py-4">@if (isset($user->visits) && $user->visits->isNotEmpty() && $lastVisit = $user->visits->last())
                        @if ($lastVisit->logout)
                            {{ $lastVisit->logout->format('F d, Y H:i:s') }} 
                            {{ $lastVisit->logout->diffForHumans() }}
                        @else
                  ...
                  <td class="px-6 py-4">{{ \Carbon\Carbon::parse($user->created_at)->format('F d, Y H:i:s') }} ({{ \Carbon\Carbon::parse($user->created_at)->diffForHumans() }})</td>
                  <td class="px-3 py-3 flex gap-2">
                      <a href="#" class="font-sm text-blue-600 dark:text-blue-500 hover:underline" title="Edit User Credendtial">
                        <button onclick="openUpdateModal(this)" data-user-id="{{ $user->id }}" type="button" class="text-white bg-sky-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-1/2 sm:w-auto px-2 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" data-modal-target="update-pass-modal"  data-modal-toggle="update-pass-modal">
                        <i class="fas fa-key"></i> 
                    </button>

                      </a>
                      <a href="#" title="Delete Account Permanently" class="font-sm text-blue-600 dark:text-blue-500 hover:underline">
                      <button onclick="openDeleteModal(this)" data-user-id="{{ $user->id }}" type="button" class="text-white bg-red-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-1/2 sm:w-auto px-2 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"  data-modal-target="userdelete-popup-modal"  data-modal-toggle="userdelete-popup-modal">
...
<!-- <section class="bg-white w-full h-full dark:bg-gray-900">
  <div class="py-10 px-5 mx-auto  w-[100%] shadow-[0px_0px_17px_-1px_rgba(120,120,120,1)] border border-gray-300 rounded-[16px]"> -->
      ...
      <form  class="mx-auto w-full" method="POST" action="{{ route('user.register') }}">
      @csrf
      <!-- Group -->
...
@endsection
@section('title','User Account Table')